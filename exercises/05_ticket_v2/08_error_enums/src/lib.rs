...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { error: "Description cannot be empty".to_string()})
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { error: "Description cannot be longer than 500 bytes".to_string()})
        }
